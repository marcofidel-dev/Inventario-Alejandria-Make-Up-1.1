...
- **Descuentos.** ¿Por línea o por venta? ¿Porcentaje o monto? ¿Quién autoriza? Son columnas en `VentaItem` o en `Venta`.
...
- **Búsqueda en el POS.** Sin lector de código de barras, cómo encuentra un producto es lo que define si la pantalla se siente fluida. Búsqueda incremental por nombre, marca y tono sobre el catálogo ya cargado en memoria. *Ya construida para la pantalla de catálogo (ver sección 9); queda reusarla en el cobro.*
