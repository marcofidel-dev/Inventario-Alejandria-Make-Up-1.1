...
const CASOS = [
  ...
  {
    regla: 'abrir la caja no envía ninguna base inicial',
    archivo: ENDPOINTS,
    pruebas: 'pruebas/Caja.prueba.jsx',
    debeCaer: 'abre enviando una petición sin cuerpo',
    romper: sustituir("abrir: () => api.post('/api/v1/caja/sesiones'),",
      "abrir: () => api.post('/api/v1/caja/sesiones', { baseInicial: 0 }),"),
  },
  {
    regla: 'la pantalla de apertura no pide ningún campo',
    archivo: CAJA,
    pruebas: 'pruebas/Caja.prueba.jsx',
    debeCaer: 'lo primero es abrirla, con un solo botón y ningún campo',
    romper: sustituir('<h2>Abrir la caja</h2>',
      '<h2>Abrir la caja</h2><Campo etiqueta="Base inicial" />'),
  },
  {
    regla: 'el cierre no pide ni envía una base para mañana',
    archivo: CERRAR_CAJA,
    pruebas: 'pruebas/Caja.prueba.jsx',
    debeCaer: 'el cierre no pide ni envía una base para mañana',
    romper: sustituir('montoRetirado: Number(montoRetirado) || 0,\n      }))',
